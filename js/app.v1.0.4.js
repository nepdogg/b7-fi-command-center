...
/* ===== B7 FI COMMAND CENTER v1.0.4 — STABLE SHELL / MORNING QUICK UPDATE / FAMILY FLOW GATES ===== */
(function(){'use strict';
const VERSION='1.0.4';
const $=(s,r=document)=>r.querySelector(s), $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
const esc104=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
function listTools104(){try{return Array.isArray(window.tools)?window.tools:(Array.isArray(tools)?tools:[])}catch(e){return[]}}
function state104(){try{return window.state||state}catch(e){return {}}}
function quarter104(){try{return String(window.getB7ActiveQuarter?.()||window.quarterLabel?.()||quarterLabel?.()||'CY26Q3').toUpperCase()}catch(e){return'CY26Q3'}}
function stamp104(){window.B7_APP_VERSION=VERSION;window.VERSION=VERSION;document.title=`B7 FI Command Center v${VERSION}`;const v=$('#appVersionLabel');if(v)v.textContent=`B7 FI COMMAND CENTER V${VERSION}`}
function ordered104(){try{if(typeof orderedMorningTools==='function')return orderedMorningTools()}catch(e){}return listTools104().filter(t=>!['Shipped','Archive'].includes(String(t.quarterStatus||t.status||'')))}
function route104(t){try{return typeof routeFor==='function'?routeFor(t):[]}catch(e){return[]}}
function routeOptions104(t,selected){return route104(t).map(x=>`<option value="${esc104(x[0])}" ${String(x[0])===String(selected||'')?'selected':''}>${esc104(x[0])} — ${esc104(x[1]||'')}</option>`).join('')}
function leadTasks104(){try{return typeof activeLeadTasks==='function'?activeLeadTasks().filter(x=>x.countProgress!==false):[]}catch(e){return[]}}
function leadOptions104(selected){return `<option value="">Not Set</option>`+leadTasks104().map(x=>`<option value="${esc104(x.id)}" ${String(x.id)===String(selected||'')?'selected':''}>${esc104(x.label)}</option>`).join('')}
function microLabel104(t){try{const m=typeof microScheduleInfo==='function'?microScheduleInfo(t):null;return m?.label||'TARGET NOT SET'}catch(e){return'TARGET NOT SET'}}
function setStatusTheme104(){document.body.dataset.center='status';document.body.dataset.theme='statuscenter';document.documentElement.style.setProperty('--page-accent','#f28c28');document.documentElement.style.setProperty('--page-accent-rgb','242,140,40');document.documentElement.style.setProperty('--center-color','#f28c28');document.documentElement.style.setProperty('--center-rgb','242,140,40');const h=$('#headerPageTitle');if(h)h.textContent=`STATUS CENTER — ${quarter104()}`;$$('.main-nav .nav-btn').forEach(b=>b.classList.toggle('active',b.dataset.view==='statuscenter'))}
function setToolbarEdit104(){const bar=$('#floatingActions');if(!bar)return;const right=$('.v854-actions-right',bar)||bar.lastElementChild;if(!right)return;right.innerHTML='';const cancel=document.createElement('button'),saveBtn=document.createElement('button');cancel.className='btn';cancel.textContent='CANCEL';cancel.onclick=()=>window.setView('statuscenter');saveBtn.className='btn primary';saveBtn.textContent='SAVE';saveBtn.onclick=saveMorning104;right.append(cancel,saveBtn)}
function renderMorning104(){
  stamp104();setStatusTheme104();
  const app=$('#app');if(!app)return;
  const list=ordered104();
  app.innerHTML=`<section class="v104-morning-editor"><div class="v104-morning-head"><div><h2>MORNING / SHIFT QUICK UPDATE</h2><p>Update each master Tool once. Current FI, Micro Schedule, and Lead/Admin are independent authoritative fields used throughout the Command Center.</p></div><div class="v104-morning-count">${list.length} SYSTEMS</div></div><div class="v104-morning-list">${list.map((t,i)=>`<article class="v104-morning-card" data-v104-tool="${esc104(t.id||t.utid)}"><div class="v104-card-id"><span>${i+1}</span><div><b>${esc104(t.id||t.utid)}</b><small>${esc104(t.codename||'')} · ${esc104(t.model||'')}</small></div></div><div class="v104-core-grid"><label>ACTUAL FI CHECKLIST<select class="v104-current">${routeOptions104(t,t.checklist)}</select></label><label>MICRO SCHEDULE TARGET<select class="v104-micro"><option value="">Target Not Set</option>${routeOptions104(t,t.microTargetChecklist)}</select><small>${esc104(microLabel104(t))}</small></label><label>LEAD / ADMIN CURRENT STEP<select class="v104-lead">${leadOptions104(t,t.currentLeadAdminTask)}</select></label><label>LOCATION<div class="v104-inline"><select class="v104-room">${['CR1','CR2','CR3'].map(x=>`<option ${x===t.room?'selected':''}>${x}</option>`).join('')}</select><input class="v104-bay" placeholder="Bay" value="${esc104(t.bay||'')}"></div></label><label>SW VERSION<input class="v104-sw" value="${esc104(t.sw||'')}"></label><label>LAMP HOURS<input class="v104-lamp" inputmode="numeric" value="${esc104(String(t.lamp??''))}"></label><label>LAMP STATE<select class="v104-lamp-state"><option ${t.lampState!=='OFF'?'selected':''}>ON</option><option ${t.lampState==='OFF'?'selected':''}>OFF</option></select></label><label>SHIP DATE<input class="v104-ship" type="date" value="${esc104(t.ship||'')}"></label></div><div class="v104-notes-grid"><label>LATEST STATUS<textarea class="v104-status" rows="4">${esc104(t.activity||'')}</textarea></label><label>NOTES<textarea class="v104-notes" rows="4">${esc104(t.notes||'')}</textarea></label></div></article>`).join('')}</div></section>`;
  setToolbarEdit104();window.scrollTo(0,0);
}
function saveMorning104(){
  const at=new Date().toISOString();let changed=0;
  $$('.v104-morning-card').forEach(row=>{const id=row.dataset.v104Tool,t=listTools104().find(x=>String(x.id||x.utid)===String(id));if(!t)return;
    const ck=$('.v104-current',row)?.value||t.checklist||'', micro=$('.v104-micro',row)?.value||'', lead=$('.v104-lead',row)?.value||'';
    t.checklist=ck;t.microTargetChecklist=micro;t.microTargetUpdatedAt=at;t.currentLeadAdminTask=lead;
    t.room=$('.v104-room',row)?.value||t.room;t.bay=$('.v104-bay',row)?.value?.trim()||'';t.sw=$('.v104-sw',row)?.value?.trim()||'';
    t.lamp=Number(String($('.v104-lamp',row)?.value||'').replace(/[^0-9.]/g,''))||0;t.lampState=$('.v104-lamp-state',row)?.value||'ON';
    t.ship=$('.v104-ship',row)?.value||'';t.activity=$('.v104-status',row)?.value||'';t.notes=$('.v104-notes',row)?.value||'';t.lastUpdatedAt=at;changed++;
  });
  const st=state104();st.morningLastUpdatedAt=at;
  try{if(typeof save==='function')save();else window.save?.()}catch(e){console.error(e)}
  try{document.dispatchEvent(new CustomEvent('b7fi:tool-records-updated',{detail:{reason:'morning-quick-update',count:changed}}))}catch(e){}
  window.setView('statuscenter');
}
function wireMorningButton104(){
  const bar=$('#floatingActions');if(!bar)return;
  const b=$$('button',bar).find(x=>/EDIT WEEKDAY MORNING STATUS|UPDATE WEEKDAY MORNING STATUS/i.test(String(x.textContent||'')));
  if(!b||b.dataset.v104Wired)return;
  const n=b.cloneNode(true);n.dataset.v104Wired='1';n.onclick=e=>{e.preventDefault();e.stopPropagation();renderMorning104()};b.replaceWith(n);
}
function ensureUpdateLive104(){
  const active=$('.main-nav .nav-btn.active');if(active?.dataset.view!=='toolcenter')return;
  const app=$('#app');if(!app||$('.v837-tool-dashboard',app))return;
  if(/TOOL COUNTDOWN/i.test(app.textContent||'')){
    const b=$$('#floatingActions button').find(x=>/^[A-Z0-9]+\s+TOOLS$/i.test(String(x.textContent||'').trim()));
    if(b&&!b.dataset.v104Auto){b.dataset.v104Auto='1';b.click()}
  }
}
function gate104(state,label){const g=document.createElement('span');g.className=`v104-flow-gate ${state}`;g.title=label;g.setAttribute('aria-label',label);g.innerHTML='<span>➜</span>';return g}
function flow104(){
  if(document.body.dataset.center!=='home')return;
  $$('.v861-family-row').forEach(row=>{
    const k=$('.v861-family-kpis',row);if(!k)return;
    $$('.v103-flow-arrow,.v104-flow-gate',row).forEach(x=>x.remove());
    const cards=Array.from(k.children).filter(x=>x.classList.contains('v861-family-kpi'));if(cards.length<5)return;
    const val=i=>Number(cards[i]?.querySelector('b')?.textContent||0);
    const waiting=val(1),infi=val(2),packing=val(3);
    k.insertBefore(gate104('green','Tool family enters quarter total'),cards[0]);
    cards[0].after(gate104('green','Quarter total established'));
    cards[1].after(gate104(waiting===0?'green':'red',waiting===0?'All tools cleared Waiting FI':'Tools remain Waiting FI'));
    cards[2].after(gate104(waiting===0&&infi===0?'green':'red',waiting===0&&infi===0?'All tools cleared FI':'Tools remain before Packing'));
    cards[3].after(gate104(waiting===0&&infi===0&&packing===0?'green':'red',waiting===0&&infi===0&&packing===0?'All tools cleared Packing':'Tools remain before Shipped'));
  });
}
function post104(){stamp104();wireMorningButton104();ensureUpdateLive104();flow104();}
const prevSetView104=window.setView;
window.setView=function(){const r=prevSetView104?.apply(this,arguments);requestAnimationFrame(()=>{post104();setTimeout(post104,60)});return r};try{setView=window.setView}catch(e){}
function boot104(){stamp104();requestAnimationFrame(()=>{post104();setTimeout(post104,120);setTimeout(post104,450)})}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot104,{once:true});else boot104();
})();
